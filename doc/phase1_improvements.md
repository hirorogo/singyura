...
def _rollout_policy_action(self, state):
    ...
    if ENABLE_ADAPTIVE_ROLLOUT:
        # AI同士の対戦を想定した戦略
        # 端優先（A/K）とSafe優先の組み合わせで戦略的優位性を確保
...
#### 適応的ロールアウトの効果
1. **AI同士の対戦に最適化**
   - 端優先（A/K）：トンネルルールを活用して相手を封鎖
   - Safe優先：連続して出せる札を優先してロックを継続

2. **戦略の最適化**
   - PIMC法のプレイアウトフェーズで、実際の大会環境（AI同士）を正確に模擬
   - ランダムAIではなく、戦略的なAIを想定した動き
   - 端優先・Safe優先の組み合わせで、相手の選択肢を効果的に制限
