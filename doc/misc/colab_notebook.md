...
〇競技ルール 一般的な7並べのルールにトンネルというローカルルールを追加したもの。 ・3人対戦(足りない枠はランダムにカードを出すAIが枠を埋める) ・4スーツのA~Kの52枚を用いる(ジョーカーは用いない) ・勝利条件は「カードを最も早く使い切る」もしくは「他プレイヤーが全員失格となる」 ・1人当たりパスは3回まで ・4回目のパスで失格(バースト)、手札をすべて盤面に出す ・【トンネル】カードがAまで出た場合、そのスーツはKからしか出すことができない。Kが出た時は同様にAからしか出せない。